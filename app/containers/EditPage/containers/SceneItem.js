...
class Sceneitemspe extends Component {
    ...
    render() {
        ...
        return (
            <div id={`scene-to-drag-${item.id}`} style={{opacity:(hover && isDragging)? 0.5 : 1}} draggable onDrop={this.onDrop.bind(this)} onDragLeave={this.onDragLeave.bind(this)} onDragEnd={this.onDragEnd.bind(this)} onDragOver={this.onDragOver.bind(this)} onDragStart={this.onDrag.bind(this)} className={styles.sceneContainer} key={item.id} onContextMenu={(e) => onSceneContext(e, item)} onClick={() => sceneClickHandler(item.id)}>
